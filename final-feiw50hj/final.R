...
name <- "Yibing Chen"
github.name <- "Chenyb"
email.address <- "chenyb@berkeley.edu"
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
v <- as.numeric(sample(rep(1:6, each=100)))
...
# Diet 1 at time 0
cw1 <- subset(ChickWeight, ChickWeight$Diet == 1 & ChickWeight$Time == 0)

  
# [1 pt]
# Create [cw2] a subset of ChickWeight with only Chicks that
# weighed more than 41 at Time 0
cw2 <- subset(ChickWeight, ChickWeight$weight > 41 & ChickWeight$Time == 0 )

  
# [1 pt]
# Create [cw3] a single number, the average chick weight at time 0
cw3 <- mean(subset(ChickWeight, ChickWeight$Time == 0)$weight)
  

# [1 pt]
# Create [cw4] a random subset of 200 ChickWeight observations (any 200)
cw4 <- subset(ChickWeight, ChickWeight$nrow ==sample(1:578, 200))
...
# 
preset <- subset(infants, infants$ed == "College")
t2 <- table(preset$smoke,preset$number)
...
#  
olderThan <- subset(infants, infants$age >= 35)$bwt 
under <- subset(infants, infants$age <= 35)$bwt 
bdiff <- mean(olderThan)-mean(under)
...
plot(y = mtcars$disp, x = mtcars$wt, xlab="wt", ylab="disp",col=as.numeric(mtcars$am + 1))
...
hist(subset(WorldBank, WorldBank$lending == "IDA")$GDP.per.capita.Current.USD, prob=T, xlab="GDP",breaks = 250, main = "GDP of :IDA")
...
hist(rain[[i]], xlab = paste("season", i))
}
...
GenNorm <- function(mean, sd, n, plot.hist = TRUE){
  vector = sample(rnorm(n = n, mean = mean , sd = sd))
  if (plot.hist == TRUE) {
    hist(vector)
  }
  return (vector)
...
PermDiff <- function(cases, controls, k=5000){
  ...
  total <- sum(newsample)
  for(i in 1:k){ 
  newcases <- sample(newsample, n)
  totalnewcases <- sum(newcases)
  totalnewcontrols <- total - totalnewcases
  meancases = mean(newcases)
  meancontrol = totalnewcontrols/length(controls)
  Diff[i] = meancases - meancontrol
  }
  return (Diff)
...
NumJackpot <- function(k, B){
  n.jackpot <- c()
  for(j in 1 : B){
  winnumber <- sample(1:19, 3)
  count <- 0
  for(i in 1 : k){
    number <- sample(1:19, 3)
    if (number[1] == winnumber[1] &&number[2] == winnumber[2]&&number[3] == winnumber[3]){
      count = count +1
    } else if ( (number[1] == winnumber[2] &&number[2] == winnumber[3]&&number[3] == winnumber[1])){
      count = count +1
    }else if ( (number[1] == winnumber[3] &&number[2] == winnumber[1]&&number[3] == winnumber[2])){
      count = count +1
    }else if ( (number[1] == winnumber[1] &&number[2] == winnumber[3]&&number[3] == winnumber[2])){
      count = count +1
    }else if ( (number[1] == winnumber[2] &&number[2] == winnumber[1]&&number[3] == winnumber[2])){
      count = count +1
    }else if ( (number[1] == winnumber[3] &&number[2] == winnumber[3]&&number[2] == winnumber[1])){
      count = count +1
    }
  }
