...
dice_sum <- function(k=2, B=100){
  return(sapply(1:B, function(x) sum(sample.int(6, k, replace=TRUE))))
}
...
updateDate <- function(dates, old.yr) {
  return(updated.dates <- gsub(old.yr, "2015", dates[grep(old.yr, dates)]))
}
...
abbreviate <- function(vector, k){
  ...
  return(as.vector(sapply(vector, function(x) substr(x, 1, k))))
}
